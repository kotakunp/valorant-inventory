...
4. `POST /store/v3/storefront/{puuid}` (body `{}`) → VP price map from daily offers, featured bundles (`BasePrice`), and night-market standard `Cost` (not discount). The old global `GET /store/v1/offers/` is **gone** (404 on every variant). Accessory-store items are Kingdom-Credit priced and excluded from the VP map — their KC prices + items (sprays/buddies/cards/titles) are surfaced separately by `buildStoreSection` in the store strip.
